...
//Pages
$query = "SELECT message_id, title, body, creation_date FROM tbl_alerts_message msg WHERE user_id = $param_session_user_user_id AND status IN(0,9) AND message_id IN (SELECT message_id FROM tbl_alerts_message_log WHERE message_id = msg.message_id) AND is_scheduled = 0 ORDER BY creation_date DESC";
