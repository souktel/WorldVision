...
//*Special for Morocco NEF system
// * if mobile number is from Morocco, Reroute the request to Morocco system
if(substr($req_mobile, 0,3) == '212' || substr($req_mobile, 0,5) == '00212' || substr($req_mobile, 0,4) == '+212') {
    $morocco_url = "http://nef.mysouktel.com/modules/sms/index.php?from=$req_mobile&text=$req_msg";
    $morocco_ch = curl_init();
    curl_setopt($morocco_ch, CURLOPT_URL, $morocco_url);
    curl_setopt($morocco_ch, CURLOPT_FOLLOWLOCATION, 1);
    curl_setopt($morocco_ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($morocco_ch, CURLOPT_G, 1);
    curl_exec($morocco_ch);
    curl_close($morocco_ch);
    exit;
}
// */
