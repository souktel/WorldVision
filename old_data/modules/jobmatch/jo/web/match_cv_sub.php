...
su.name, 
c.reference_code, 
(SELECT name FROM tbl_ref_city_title WHERE city_id = c.city AND language_id = $param_session_sys_language) AS cityname, 
c.addition_date, 
(
ifnull((SELECT $se FROM tbl_js_mini_cv WHERE cv_id = c.cv_id AND major = (SELECT major FROM tbl_jo_job_vacancy WHERE reference_code = '$reference' AND user_id = $param_session_user_user_id)),0)+
ifnull((SELECT $el FROM tbl_js_mini_cv WHERE cv_id = c.cv_id AND education_level >= (SELECT education_level FROM tbl_jo_job_vacancy WHERE reference_code = '$reference' AND user_id = $param_session_user_user_id)),0)+
ifnull((SELECT $ex FROM tbl_js_mini_cv WHERE cv_id = c.cv_id AND experience >= (SELECT experience FROM tbl_jo_job_vacancy WHERE reference_code = '$reference' AND user_id = $param_session_user_user_id)),0)+
ifnull((SELECT $lc FROM tbl_js_mini_cv WHERE cv_id = c.cv_id AND city = (SELECT city FROM tbl_jo_job_vacancy WHERE reference_code = '$reference' AND user_id = $param_session_user_user_id)),0)+
ifnull((SELECT $wh FROM tbl_js_mini_cv WHERE cv_id = c.cv_id AND hours_range = (SELECT hours_range FROM tbl_jo_job_vacancy WHERE reference_code = '$reference' AND user_id = $param_session_user_user_id)),0)
) AS wgt 
...
<table border="0" width="680" cellspacing="3" cellpadding="6" style="border-collapse: collapse">
    ...
    <tr>
        ...
        <td width="275"><font face="Trebuchet MS" size="2"><a href="view_cv.php?cid=<?php echo $rs_cv_id;?>"><?php echo $rs_uname;?><font dir="rtl">&nbsp;<?php echo "($rs_wgt)";?>&nbsp;</font></a></font></td>
