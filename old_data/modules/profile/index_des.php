...
if($row = mysql_fetch_array($rs))
{
    ...
    if($rs_usertype=="1")
    {
        $query = "SELECT * FROM tbl_sys_user_ind WHERE user_id = $param_session_user_user_id";
    ...
    {
        $query = "SELECT * FROM tbl_sys_user_nind WHERE user_id = $param_session_user_user_id";
        $rs2 = mysql_query($query,$dbid1);
        if($row2 = mysql_fetch_array($rs2))
        {
            $rs_phone = $row2['phone'];
